      ...
      module Executors
        class Agent < Core::Executors::Base
          ...
          private
            ...
            def build_tool_classes(state, agent)
            ...
            def run_agent_loop(state, agent, prompt, tool_classes)
              ...
              loop do
                ...
                response.tool_calls.each do |tool_call|
                  tool_name = tool_call.name rescue Utils.fetch(tool_call, :name)
                  arguments = tool_call.arguments rescue (Utils.fetch(tool_call, :arguments) || {})
            ...
            def execute_tool_call(state, tool_name, arguments)
